<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.jiaoshiName" type="text" placeholder="教室名称"></input>
		</view>
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	this.jiaoshiTypesList = jiaoshiTypes.data.list;
	   		//当前表的 楼层 字段 字典表查询方法
		  	let loucengTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'louceng_types'
		  	});
		  	this.loucengTypesList = loucengTypes.data.list;
	   		//当前表的 教室状态 字段 字典表查询方法
		  	let jiaoshiOrderTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'jiaoshi_order_types'
		  	});
		  	this.jiaoshiOrderTypesList = jiaoshiOrderTypes.data.list;
